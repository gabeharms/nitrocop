...
def test_renders_only_present_helper_scripts():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        (root / "scripts" / "agent" / "detect_changed_cops.py").write_text("print('ok')\n")
        ...
        assert "`scripts/agent/detect_changed_cops.py`" in section
        ...
        assert "python3 scripts/agent/detect_changed_cops.py --base origin/main --head HEAD" in section
