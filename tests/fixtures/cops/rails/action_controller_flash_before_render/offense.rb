...
# FN fix: flash inside case/when before render after the case block
class PaymentsController < ApplicationController
  def payment_completion
    ...
    render :payment_completion
  end
end

# FN fix: flash inside case/when with implicit render (no explicit render/redirect)
class StatusController2 < ApplicationController
  def show_status
    case @status
    when :error
...
# FN fix: flash inside case/when with render in else clause of case
class ReportsController < ApplicationController
  def generate
    case @report.format
    when :pdf
      flash[:notice] = "Generating PDF"
      ^^^^^ Rails/ActionControllerFlashBeforeRender: Use `flash.now` before `render`.
    else
      flash[:notice] = "Default format"
      ^^^^^ Rails/ActionControllerFlashBeforeRender: Use `flash.now` before `render`.
    end
    render :generate
  end
...
# FN fix: flash in case/when with respond_to containing render after
