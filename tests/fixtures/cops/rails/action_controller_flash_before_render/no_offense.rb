...
# FP fix: flash inside explicit begin/rescue before respond_to with both render AND redirect.
# RuboCop finds :rescue ancestor, rescue.right_siblings is empty → no render → no offense.
class GradesController < ApplicationController
  def save_grade_and_comment_for_reviewer
    ...
    respond_to do |format|
      format.js { render action: 'save_grade.js.erb', layout: false }
      format.html { redirect_to controller: 'reports', action: 'response_report' }
    end
  end
end

# FP fix: flash in begin/rescue body before respond_to with render (json) and implicit html render
...
# FP fix: flash before explicit begin/rescue that contains redirect_to
...
# No offense: flash inside case/when with redirect_to in same branch
