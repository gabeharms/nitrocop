            ...
            if read_name.as_slice() == write_name {
                ...
                if let Some(corr) = corrections.as_mut() {
                    ...
                    let replacement = if let Some(and_node) = value.as_and_node() {
                        format!("{} &&= {}", write_name, Self::node_source(source, &and_node.right()))
                    } else if let Some(or_node) = value.as_or_node() {
                        format!("{} ||= {}", write_name, Self::node_source(source, &or_node.right()))
                    } else {
                        return;
                    };
...
mod tests {
    ...
    crate::cop_autocorrect_fixture_tests!(SelfAssignment, "cops/style/self_assignment");
}
