...
impl Cop for WhileUntilModifier {
    ...
    fn check_node(
        ...
        if let Some(corrs) = corrections.as_mut() {
            ...
            let mut replacement =
                format!("{}{} {} {}", indent, body_trimmed, keyword, pred_trimmed);

            if has_first_line_comment {
                for comment in _parse_result.comments() {
                    let (comment_line, _) =
                        source.offset_to_line_col(comment.location().start_offset());
                    if comment_line == kw_line {
                        replacement.push(' ');
                        replacement
                            .push_str(&String::from_utf8_lossy(comment.location().as_slice()));
                        break;
                    }
                }
            }
